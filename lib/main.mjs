...
function handleMidiMessage(msg) {
  ...
  // TODO if i allow shifting octaves, change the diff
  const noteId = noteNumber - 48
  const noteToPlay = pianoNotes[noteId]
  switch (cmd) {
    case 144:
      play(noteId, noteToPlay.letter + noteToPlay.octave.toString())
      break
    case 128:
      stop(noteId, noteToPlay.letter + noteToPlay.octave.toString())
      break
    default:
      break
  }
...
function playScale() {
  ...
  const channel = initChannel(selectedScale[0].frequency)
  channel.osc.start()
...
function playChord() {
  ...
  noteIds.forEach((id) => {
    const channel = initChannel(pianoNotes[id].frequency)
    channels[id] = channel
...
window.onload = function () {
  ...
  keyboard.keyDownCallback = (id, note) => {
    if (!pedalOn) {
      play(id, note)
    } else {
      currentNotes.has(note) ? stop(id) : play(id, note)
    }
  }
  keyboard.keyUpCallback = (id, note) => {
    if (!pedalOn) {
      stop(id, note)
    }
  }
