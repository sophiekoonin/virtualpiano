...
const SingleOctaveIntervals = Object.freeze({
  C: -9,
  "C#": -8,
  D: -7,
  "D#": -6,
...
export const notes = Object.keys(SingleOctaveIntervals)

// generate an array containing all 12 semitones in octave, from root to root + 8
export function generateOctaves(rootNote, startingOctave = 4, numOctaves = 1) {
...
export function calcDistance(rootNote, otherNote) {
  ...
  const rootNoteDistanceFromA = SingleOctaveIntervals[rootNote]
  return SingleOctaveIntervals[otherNote] - rootNoteDistanceFromA
}

// takes a note and its octave, e.g. A, 4, and returns frequency.
export function calcFrequency(noteLetter, octave) {
  const singleOctaveInterval = SingleOctaveIntervals[noteLetter] // A -> C is -9

  const offset = (octave - 4) * 12
  const semitoneInterval = singleOctaveInterval + offset

  // f = 440Hz * 2^n/12
  const freq = A4 * Math.pow(2, semitoneInterval / 12)
  // round to 1 d.p.
